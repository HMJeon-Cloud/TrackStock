...
function guideScrollTo(id, tries) {
  if (typeof revealEl === "function") revealEl(id);
  if (!id) return;
  tries = tries || 0;
  setTimeout(function () {
    var el = $(id);
...
var GUIDE_METHODS = [
  ...
  },
  {
    ...
    steps: [
      ...
      { t: "비교 탭에서 거치식 vs 적립식을 비교합니다", d: "투자 방식을 '적립식'으로 바꾸고 최악의 타이밍(고점) 프리셋으로 시작해 보세요. 고점에 시작해도 적립식이면 결과가 어떻게 달라지는지 보입니다.", go: { tab: "compare", id: "compareCard", needSymbol: false }, label: "장바구니 비교" },
  ...
  },
  {
    ...
    steps: [
      ...
      { t: "비교 탭의 상관계수 행렬을 읽습니다", d: "1에 가까우면 같이 움직임(분산 효과 없음), 0 근처면 따로 움직임, 음수면 반대로 움직임(최고의 짝). 주식-장기채가 대체로 0~-0.3, 주식-금 0 근처, 주식-나스닥 0.9 이상입니다.", go: { tab: "compare", id: "cmpCorr", needSymbol: false }, label: "상관계수 행렬" },
  ...
  },
  {
    ...
    steps: [
      { t: "차트에 이동평균선을 켜고 최근 종가와 240일선 위치를 봅니다", d: "20일(주황)·60일(초록)·240일(보라). 주가 > 20일선 > 60일선 > 240일선 순으로 위에서 아래로 놓이면(정배열) 강한 상승 추세, 그 반대(역배열)면 하락 추세입니다.", go: { tab: "single", sub: "analysis", id: "chartCard" }, label: "메인 차트 (이동평균선 체크)" },
  ...
  },
  {
    ...
    steps: [
      ...
      { t: "지지·저항선 표와 차트 오버레이를 봅니다", d: "최근 종가가 지지선 바로 위인지, 저항선 바로 아래인지. 터치 횟수 2회 이상인 선만 의미 있습니다.", go: { tab: "single", sub: "analysis", id: "techCard" }, label: "지지/저항선" },
  ...
  },
  {
    ...
    steps: [
      ...
      { t: "실적 추이에서 매출·영업이익이 늘고 있는지 봅니다", d: "분기/연간 토글. 전망치(파란색)까지 우상향인지, 영업이익률이 유지되는지.", go: { tab: "single", sub: "analysis", id: "fundFinance" }, label: "실적 추이" },
